-- =====================================
-- 🔐 S1GT JB CHEATS - FIXED SPEED + BOOST + SCROLLING + TURNING
-- =====================================
...
-- Speed ScrollingFrame
local speedScroll = Instance.new("ScrollingFrame", contentFrame)
speedScroll.Size = UDim2.new(1,-20,1,-70)
speedScroll.Position = UDim2.fromOffset(10,40)
speedScroll.BackgroundTransparency = 1
speedScroll.ScrollBarThickness = 8
speedScroll.ScrollBarImageColor3 = Color3.fromRGB(0,170,255)
speedScroll.CanvasSize = UDim2.fromOffset(0,460)
speedScroll.Visible = true

local speedContent = Instance.new("Frame", speedScroll)
speedContent.Size = UDim2.new(1,0,0,460)
speedContent.BackgroundTransparency = 1

-- Turn Content
local turnContent = Instance.new("Frame", contentFrame)
turnContent.Size = UDim2.new(1,-20,1,-70)
...
speedSliderBack.Position = UDim2.fromOffset(10,80)
...
speedLabel.Position = UDim2.fromOffset(10,110)
...
speedBox.Position = UDim2.fromOffset(190,110)
...
speedBindLabel.Position = UDim2.fromOffset(10,140)
...
speedBindBox.Position = UDim2.fromOffset(190,140)
...
-- Boost Section
local boostLabel = Instance.new("TextLabel", speedContent)
boostLabel.Size = UDim2.fromOffset(240,24)
boostLabel.Position = UDim2.fromOffset(10,180)
boostLabel.BackgroundTransparency = 1
boostLabel.Text = "Boost (works only when Speed OFF)"
...
jumpBoostLabel.Size = UDim2.fromOffset(240,24)
jumpBoostLabel.Position = UDim2.fromOffset(10,310)
jumpBoostLabel.BackgroundTransparency = 1
jumpBoostLabel.Text = "Jump Boost"
jumpBoostLabel.Font = Enum.Font.GothamBold
jumpBoostLabel.TextSize = 13
jumpBoostLabel.TextColor3 = Color3.new(1,1,1)
jumpBoostLabel.TextXAlignment = Enum.TextXAlignment.Center

local jumpBoostBindLabel = Instance.new("TextLabel", speedContent)
jumpBoostBindLabel.Size = UDim2.fromOffset(160,24)
jumpBoostBindLabel.Position = UDim2.fromOffset(10,340)
jumpBoostBindLabel.BackgroundTransparency = 1
jumpBoostBindLabel.Text = "Jump Key:"
jumpBoostBindLabel.Font = Enum.Font.Gotham
jumpBoostBindLabel.TextSize = 12
jumpBoostBindLabel.TextColor3 = Color3.new(1,1,1)

local jumpBoostBindBox = Instance.new("TextButton", speedContent)
jumpBoostBindBox.Size = UDim2.fromOffset(60,24)
jumpBoostBindBox.Position = UDim2.fromOffset(190,340)
...
-- ============= TURNING =============
-- (Unchanged - same UI and physics with cleanup on disable)

-- ============= LOGIC & FIXED PHYSICS =============
-- (Sliders, keybinds, toggle UI - unchanged)

-- FIXED PHYSICS - Speed and Boost now work correctly
RunService.RenderStepped:Connect(function(dt)
	...
	for _, v in pairs(workspace.Vehicles:GetChildren()) do
		...
		and v.Seat.PlayerName.Value == player.Name and v.PrimaryPart then
			...
			if isBoosting and tick() >= boostEndTime then
				isBoosting = false
			end

			local usingBoost = isBoosting and not speedEnabled

			local targetSpeed = usingBoost and (JB_Speed * boostMultiplier) or JB_Speed
			local targetMaxForce = usingBoost and (MAX_FORCE * boostMultiplier) or MAX_FORCE
			...
			if dir ~= 0 and math.abs(forwardSpeed) < maxStudSpeed then
				currentForce = math.min(currentForce + targetMaxForce * ACCEL_RATE * dt, targetMaxForce)
			else
				currentForce = math.max(currentForce - targetMaxForce * DECEL_RATE * dt, 0)
			end
